...
function TabFace({
  ...
  return (
    ...
    >
      {active ? (
        <span className="absolute inset-x-2 top-0 h-0.5 rounded-full bg-podium-yellow" />
      ) : null}
...
function MobileNavLink({
  ...
  return (
    <Link
      href={href}
      className="group relative flex min-h-11 flex-1 flex-col items-center justify-center gap-1 rounded-md px-1 py-1"
    >
...
export function MobileTabBar({
  ...
  return (
    <>
      <nav
        className={cn(
          "fixed inset-x-0 bottom-0 border-t border-white/10 bg-podium-navy/95 pb-[env(safe-area-inset-bottom,0px)] backdrop-blur-xl md:hidden",
          SHELL_Z.tab,
        )}
      >
        <div className="mx-auto flex max-w-lg items-stretch justify-around px-1 py-1.5">
          {staticNav
            ? SHELL_MOBILE_NAV.map((item) => (
                <span
                  key={item.href}
                  className="relative flex min-h-11 flex-1 flex-col items-center justify-center gap-1 rounded-md px-1 py-1"
                >
                  <TabFace
