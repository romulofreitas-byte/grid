...
export async function PATCH(
  ...
  const parsed = ignoreSchema.safeParse(await readJson(req));
  if (!parsed.success) return jsonError("Não foi possível ignorar os erros.");
  const run = await getRepo().ignoreCrmImportRunErrors(gated.userId, runId);
  if (!run) return jsonError("Importação não encontrada.", 404);
  return NextResponse.json({ run: toPublicImportRunDetail(run) });
}
