...
async function loadCrmRateInputPg(userId: string): Promise<{
  deals: CrmRateDeal[];
  calledCnpjs: number;
}> {
  const [dealRows, callRows] = await Promise.all([
    optionalQuery<{
    ...
    optionalQuery<{ n: number }>(
      `select count(distinct cnpj)::int as n
         from call_events
        where user_id = $1`,
      [userId],
    ),
  ...
  return {
    deals: dealRows.map((row) => ({
      ...
      outcome:
        row.outcome === "won" || row.outcome === "lost" ? row.outcome : "open",
      amount_cents:
        row.amount_cents == null ? null : Number(row.amount_cents),
    })),
    calledCnpjs: Number(callRows[0]?.n ?? 0),
  };
}

function loadCrmRateInputMock(userId: string): {
  deals: CrmRateDeal[];
  calledCnpjs: number;
} {
  ...
  const deals: CrmRateDeal[] = store.crm_deals
  ...
  const called = new Set(
    store.call_events.filter((row) => row.user_id === userId).map((row) => row.cnpj),
  );
  return { deals, calledCnpjs: called.size };
}
...
export async function loadCrmSuggestions(
  ...
  const input =
    getDataSource() === "supabase" && hasLiveDatabase()
      ? await loadCrmRateInputPg(userId)
      : loadCrmRateInputMock(userId);
  return suggestCrmRates(input);
}
